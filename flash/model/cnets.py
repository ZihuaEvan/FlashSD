...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True):
        ...
        # Visual Compressor parameters (configurable from config)
    ...
    def forward(
        ...
        # --- Visual Compressor path ---
        # Auto-detect: use compression if image tokens are present in input_ids
        image_mask = (input_ids == self.image_token_id)
        has_image = image_mask.any() if use_compress is None else use_compress

        if has_image and image_mask.any():
            # Split into pre-image, image, post-image regions
            start_indices = image_mask.int().argmax(dim=1)  # (B,)
            mid_lengths = image_mask.sum(dim=1)  # (B,)
            ...
            # Pre-image features: pass through directly
            pre_feats = hidden_states[pre_mask].view(batch_size, -1, dim)

            # Post-image (text) features: concat(embed, hidden) -> FC
            post_data_emb = inputs_embeds[post_mask]
            post_data = hidden_states[post_mask]
            concat = torch.cat((post_data_emb, post_data), dim=-1)
            text_embs = self.fc(concat)
            post_feats = text_embs.view(batch_size, -1, dim)

            # Image features: attention-based compression
            total_mid_tokens = mid_lengths[0].item()
            num_images = max(total_mid_tokens // self.image_token_per_image, 1)
            img_feats = hidden_states[mid_mask].view(batch_size, num_images, -1, dim)
            ...
            # Reconstruct sequence: pre + compressed + post
            hidden_states = torch.cat([pre_feats, compressed_feats, post_feats], dim=1)
            seq_length_with_past = hidden_states.shape[1] + past_key_values_length
        else:
            # No compression: simple FC path
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.view(-1, hidden_states.shape[1]).long()
...
if __name__ == "__main__":
    ...
    model = Model(config, load_emb=True, path="path/to/vicuna-7b/")
    print(model)
