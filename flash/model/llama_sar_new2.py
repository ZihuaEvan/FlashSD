...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True):
        ...
        # Hydra-style post-refinement: sequential conditioning across slots.
        # Slot i (i>=1) is refined using the embedding of the token predicted by slot i-1.
        # Each projection takes [cumulative_embed || slot_output] (2*D) -> D as a residual delta.
        
        '''
        self.hydra_projs = nn.ModuleList([
            nn.Linear(2 * config.hidden_size, config.hidden_size, bias=False)
            for _ in range(config.k - 1)
        ])
        '''
        hydra_bottleneck = config.hidden_size // 8  # 512
        self.hydra_projs = nn.ModuleList([
            nn.Sequential(
                nn.Linear(2 * config.hidden_size, hydra_bottleneck, bias=False),
                nn.ReLU(),
                nn.Linear(hydra_bottleneck, config.hidden_size, bias=False),
            )
            for _ in range(config.k - 1)
        ])

        # Dropout on Hydra delta: reduces over-reliance on conditioning signal.
        self.hydra_dropout = nn.Dropout(0.1)
        # Scheduled sampling probability: probability of using model's own prediction
        # instead of teacher-forced target_ids during training. Set externally per epoch.
        # 0.0 = pure teacher forcing; 1.0 = pure self-prediction.
        self.ss_prob = 0.0
    ...
    def _hydra_refine(self, output, target_ids=None, lm_head=None):
        """Hydra-style sequential refinement across slots.

        After the main SAR head produces predictions for all k slots in
        parallel, this method refines slots 1..k-1 by conditioning each on
        the cumulative token embeddings of previous slots' predictions.

        Scheduled Sampling: during training, with probability ``self.ss_prob``,
        each slot uses the model's own prediction (via lm_head) instead of
        the teacher-forced target_ids.  This exposes the model to imperfect
        conditioning during training, reducing the train-test gap.

        Dropout: the refinement delta is dropped out before being added to
        the slot output, reducing over-reliance on the conditioning signal.

        Args:
            output: (B, L, k, D) — raw SAR head output.
            target_ids: (B, L, k) — ground truth token IDs (training, teacher forcing).
                        target_ids[:, :, i] = token at position t+i+1.
                        For slot i's conditioning we need token at t+i = target_ids[:,:,i-1].
            lm_head: nn.Linear (V, D) — frozen LM head.
                     Used for scheduled sampling during training and always during inference.

        Returns:
            (B, L, k, D) — refined output.
        """
        B, L, k, D = output.shape

        if target_ids is None and lm_head is None:
            return output  # No conditioning available, skip refinement

        # Build refined output slot by slot
        refined_slots = [output[:, :, 0, :]]  # slot 0 unchanged (no conditioning)
        cumulative_embed = torch.zeros(B, L, D, device=output.device, dtype=output.dtype)

        for i in range(1, k):
            # Determine conditioning token for this slot.
            # Slot i is conditioned on the token at position t+i (= slot i-1's prediction target).
            use_model_pred = False
            if self.training and self.ss_prob > 0 and lm_head is not None:
                # Scheduled sampling: randomly choose teacher vs model prediction
                use_model_pred = (torch.rand(1).item() < self.ss_prob)

            if not use_model_pred and target_ids is not None:
                # Teacher forcing: use the true token at t+i
                # target_ids[:, :, i-1] = token at position t+(i-1)+1 = t+i
                prev_token = target_ids[:, :, i - 1].long()
            elif lm_head is not None:
                # Self-prediction: use slot i-1's REFINED prediction → decode → get token
                with torch.no_grad():
                    prev_logits = lm_head(refined_slots[i - 1])  # (B, L, V)
                    prev_token = prev_logits.argmax(dim=-1)  # (B, L)
            else:
                # Fallback: no conditioning available for this slot, skip refinement
                refined_slots.append(output[:, :, i, :])
                continue

            # Get embedding of the conditioning token
            with torch.no_grad():
                prev_embed = self.embed_tokens(prev_token)  # (B, L, D)

            # Accumulate embeddings: slot i sees embeddings of tokens t+1 to t+i
            cumulative_embed = cumulative_embed + prev_embed

            # Compute refinement delta with dropout
            cat_input = torch.cat([cumulative_embed, output[:, :, i, :]], dim=-1)  # (B, L, 2D)
            delta = self.hydra_projs[i - 1](cat_input)  # (B, L, D)
            delta = self.hydra_dropout(delta)  # Dropout on delta
            refined_slots.append(output[:, :, i, :] + delta)

        return torch.stack(refined_slots, dim=2)  # (B, L, k, D)
    ...
    def sar_only(self, post_fc_buffer, padding_mask=None, hidden_states_residual=None,
        ...
        Args:
            ...
            target_ids: (B, L', k) optional — for Hydra teacher forcing.
            lm_head: nn.Linear optional — for Hydra inference refinement.
        ...
        # Hydra refinement
        out = self._hydra_refine(out, target_ids=target_ids, lm_head=lm_head)
        return out

    def forward_incremental(self, hidden_states_new, inputs_embeds_new,
                            post_fc_buffer=None, lm_head=None):
        ...
        Args:
            ...
            lm_head: nn.Linear optional — frozen LM head for Hydra refinement.
        ...
        sar_out = self.sar_only(full_buffer, lm_head=lm_head)
    ...
    def forward(
        ...
        5. Hydra refinement: sequential conditioning across slots.
        6. Training: expand to (B, L, k, D); inference: rearrange to (B, L'*k, D).

        Args:
            training_format: If True, return (B, L, k, D) regardless of self.training.
                If None (default), follow self.training.
            target_ids: (B, L, k) — ground truth future token IDs for Hydra refinement
                (teacher forcing during training). Optional.
            lm_head: nn.Linear — frozen LM head for Hydra refinement during inference.
                Optional.
        """
        ...
        if use_training_format:
            ...
            # --- Step 5: Hydra refinement (sequential conditioning across slots) ---
            layer_outputs = self._hydra_refine(layer_outputs, target_ids=target_ids, lm_head=lm_head)

            return layer_outputs
        else:
            # Inference path (block-diagonal rearrange for speculative decoding)
            # Apply hydra refinement before rearranging
            layer_outputs = self._hydra_refine(layer_outputs, target_ids=target_ids, lm_head=lm_head)
