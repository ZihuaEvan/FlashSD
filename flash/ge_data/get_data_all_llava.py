...
parser.add_argument(
    "--outdir", type=str, default='./data/llava'
)
...
bigname = "path/to/llava-1.5-7b-hf"
...
def build_dataset_rank(
    ...
    ds = load_dataset(
        "json", data_files="path/to/llava_instruct_150k.json"
    )
    ds = ds["train"]
    ...
    def preprocess_function(examples, seen_ids):
        ...
        for i in range(len(examples["id"])):
            ...
            # --- Data-side constraint: enforce fixed image token count ---
            # LLaVA-1.5 should produce exactly EXPECTED_IMAGE_TOKENS image tokens per image.
            # Skip samples whose image token count deviates (e.g., LLaVA-NeXT anyresolution
            # outputs, corrupted images, or processor mis-configuration). This guarantees
            # downstream SAR/AR training sees consistent (B, num_images, 576, D) shapes.
            IMAGE_TOKEN_ID = 32000
            EXPECTED_IMAGE_TOKENS = 576
            n_img_tokens = int((input_ids == IMAGE_TOKEN_ID).sum().item())
            if n_img_tokens != EXPECTED_IMAGE_TOKENS:
                print(
                    f"[skip] image={url} has {n_img_tokens} image tokens "
                    f"(expected {EXPECTED_IMAGE_TOKENS}); skipping."
                )
                continue
