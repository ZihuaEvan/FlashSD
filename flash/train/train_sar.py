...
parser.add_argument("--basepath", type=str, default="%TARGET_PATH%")
parser.add_argument(
    "--configpath",
    type=str,
    default="./nar_config.json",
)
parser.add_argument("--lr", type=float, default=3e-5)
parser.add_argument("--bs", type=int, default=2)
parser.add_argument("--gradient-accumulation-steps", type=int, default=1)
parser.add_argument(
    "--tmpdir", type=str, default='%DATA_PATH%'
)
parser.add_argument(
    "--outdir", type=str, default="../weights"
)
parser.add_argument(
    "--cpdir", type=str, default="../weights"
)
...
from eagle_eye.model.llama_sar import Model
    ...
    for i in range(bs):
        for j in range(sl):
            ...
            for k in range(max_length):
                ...
                tmp_token = input_ids[i, single_hidden_states.shape[1] - 1 + 1 - 576]
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        with accelerator.accumulate(model):
            ...
            target_p= target_p.repeat(1,int(config.k),1)
            loss_mask= loss_mask.repeat(1,int(config.k),1)
            plogp = target_p * out_logp
            ploss = -torch.sum(torch.sum(loss_mask * plogp, 2)) / (
                loss_mask.sum() + 1e-5
            )
            target = data["target"].repeat(1,int(config.k),1)
        ...
        with torch.no_grad():
            ...
            target = target.repeat(1,config.k)
        ...
        num_batches += 1
        torch.save(model.state_dict(), f"{args.cpdir}/pytorch_model.bin")
'''
    ...
    if (epoch + 1) % train_config["save_freq"]:
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            with torch.no_grad():
                ...
                target = target.view(-1)[loss_mask.view(-1) == 1]
                topkacc = top_accuracy(out_head, target, (1,))
                for top_i in range(len(topkacc)):
                    top_3acc[top_i] += topkacc[top_i]
                total += ct
                correct += cc
